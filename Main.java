...
public class Main {
  ...
  cp.getBoard().getTile(0,6).setPiece(Wp1);cp.getBoard().getTile(1,6).setPiece(Wp2);cp.getBoard().getTile(2,6).setPiece(Wp3);cp.getBoard().getTile(3,6).setPiece(Wp4);cp.getBoard().getTile(4,6).setPiece(Wp5);cp.getBoard().getTile(5,6).setPiece(Wp6);cp.getBoard().getTile(6,6).setPiece(Wp7);cp.getBoard().getTile(7,6).setPiece(Wp8);cp.getBoard().getTile(0,7).setPiece(Wr1);cp.getBoard().getTile(1,7).setPiece(Wn1);cp.getBoard().getTile(2,7).setPiece(Wb1);cp.getBoard().getTile(3,7).setPiece(Wq);
  // cp.getBoard().getTile(4, 7).setPiece(Wk);
  cp.getBoard().getTile(5,7).setPiece(Wb2);cp.getBoard().getTile(6,7).setPiece(Wn2);cp.getBoard().getTile(7,7).setPiece(Wr2);

  //Setting all the locations of the black pieces
  cp.getBoard().getTile(0,1).setPiece(Bp1);cp.getBoard().getTile(1,1).setPiece(Bp2);cp.getBoard().getTile(2,1).setPiece(Bp3);cp.getBoard().getTile(3,1).setPiece(Bp4);cp.getBoard().getTile(4,1).setPiece(Bp5);cp.getBoard().getTile(5,1).setPiece(Bp6);cp.getBoard().getTile(6,1).setPiece(Bp7);cp.getBoard().getTile(7,1).setPiece(Bp8);cp.getBoard().getTile(0,0).setPiece(Br1);cp.getBoard().getTile(1,0).setPiece(Bn1);cp.getBoard().getTile(2,0).setPiece(Bb1);cp.getBoard().getTile(3,0).setPiece(Bq);
  // cp.getBoard().getTile(4, 0).setPiece(Bk);
  cp.getBoard().getTile(5,0).setPiece(Bb2);cp.getBoard().getTile(6,0).setPiece(Bn2);cp.getBoard().getTile(7,0).setPiece(Br2);
  ...
  System.out.println("\"Tour Guide\" Project");System.out.println("Chess is a board game for two players, referred to as white and black, each controlling an army of chess pieces, with the objective to checkmate the opponent's king. Below are some of the features of the console. Enter the corresponding numbers to access this feature at any time!");System.out.println("(1) Help");System.out.println("(2) Rules");System.out.println("(3) Draw/Forfeit");while(true)
  {
    String input = chessConsole.nextLine();
    //if 1 is entered, the console will offer help on how to play
    if (input.equals("1")) {
      System.out.println(
          "To move the pieces first click the piece you want to move, then click the square you want to move it to. If you click a tile where there is already a piece, your own piece will only move if the existing piece on the tile is a different color");
    }
